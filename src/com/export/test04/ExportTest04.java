...
public class ExportTest04 {
	public static void main(String[] args) {
		...
		try {
			out = new FileOutputStream("m:\\success4.xls");
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
		ExcelUtil<StudentVO> util = new ExcelUtil<StudentVO>(StudentVO.class);
		util.exportExcel(convertStu2VO(list), "学生信息qq", 60000, out);
	...
	private static List<StudentVO> convertStu2VO(List<Student> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			switch (stu.getClazz()) {
			case 5:
				stuVo.setClassName("五期提高班");
				break;
			case 6:
				stuVo.setClassName("六期提高班");
				break;
			case 7:
				stuVo.setClassName("七期提高班");
			default:
				break;
			}
			list2.add(stuVo);
